...
:: FYI registered KMS server address (cache) enables the system to automatically renew the license (for next 180 days) every 7 days,
:: as long as the server is online.
:: This process is same as how the legal KMS suppose to work, so no security program will flag this behavior.
...
: ======================================================================
:  This script is a fork of KMS_VL_ALL-SppExtComObjPatcher Project
:  The main project is maintained by @abbodi1406 (MDL)
:  https://forums.mydigitallife.net/posts/838808

:  Thanks to @RPO (MDL), for providing the help in making of this fork.
: ======================================================================
...
cls
title Hcore Online  Activation
if /i "%*" EQU "/u" (set Unattended=1) else (set Unattended=0)
for /f "tokens=6 delims=[]. " %%G in ('ver') do set winbuild=%%G
set "EchoRed=powershell write-host -back Black -fore Red"
set "EchoGreen=powershell write-host -back Black -fore Green"
...
reg query "%key%" /f Path /s | find /i "\Online_KMS_Activation_Script-Renewal" >nul && set "OldFiles=1" && (
if %Unattended% NEQ 1 (
if defined OldFiles (
echo.
echo ------------------------------------------------------------------------
echo   Checking: 
echo   Scheduled Task[s] to renew the Online KMS Activation is installed.
echo   It is recommended to remove it because it's not supported anymore.
echo   Press [A] to remove it and continue.
echo ------------------------------------------------------------------------
echo.
echo ----------------------------------------
...
cls
powershell -command "&{$H=get-host;$W=$H.ui.rawui;$B=$W.buffersize;$B.width=98;$B.height=160;$W.buffersize=$B;}"

if "%~f0"=="%ProgramData%\Online_KMS_Activation.cmd" echo Running: %~f0

setlocal
call :Activation_Start /u
@echo off
endlocal
...
if defined OldFiles (
if defined error_ (
%EchoRed% Error in clearing old Online KMS Renewal Task related files. - Try again. &echo.
) else (
%EchoGreen% Cleared old Online KMS Renewal Task related files successfully. &echo.
)
)
...
: ===================================================
:  Multi KMS servers integration and 1688 Port Test
:  Written by @RPO (MDL)
: ===================================================
...
if %WinBuild% GEQ 9600 powershell -nologo -command test-netconnection %KMS_IP% -port 1688 -InformationLevel Quiet | findstr /i true 1>nul
if %WinBuild% LSS 9600 powershell New-Object System.Net.Sockets.TCPClient -ArgumentList %KMS_IP%, 1688 | findstr /irC:"connected.*true" 1>nul
if %errorlevel% NEQ 0 (
set /a server_num+=1
goto :server
)
if %online_server_count%==0 (cls &echo.)
